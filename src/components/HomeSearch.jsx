...
export default function HomeSearch() {
  ...
  return (
    <>
        ...
        className='flex w-full mt-5 mx-auto max-w-[90%] border border-gray-200 px-5 py-3 rounded-full hover:shadow-md focus-within:shadow-md transtison:shadow sm:max-w-xl lg:max-w-2xl'>
            <AiOutlineSearch className='text-xl text-gray-500 mr-3 mt-0.5'/>
            <input type="text" className='flex-grow focus:outline-none'
            value={input}
            onChange={(e) => setInput(e.target.value)}/>
            <BsFillMicFill className='mt-0.5 text-lg'/>
        </form>
        <div className='flex flex-col space-y-2 sm:space-y-0 justify-center sm:flex-row mt-8 sm:space-x-4'>
            ...
            >
                {randomSearchLoading ? "Loding..." : "I am feeling lucky"}
            </button>
        </div>
    </>
  )
